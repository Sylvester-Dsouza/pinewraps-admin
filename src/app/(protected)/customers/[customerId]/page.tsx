...
export default function CustomerDetailsPage() {
  ...
  const loadCustomer = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await customerService.getCustomer(params.userId as string);
  ...
  const loadRewardPoints = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/rewards/${params.userId}`, {
        headers: {
  ...
  const handleAddPoints = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/rewards/${params.userId}/add`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${await auth.currentUser?.getIdToken()}`,
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-3xl font-bold">{customer.reward?.points || 0}</div>
                </div>
                <div className="space-y-2">
